...
public class MatterRefinerMenu extends AbstractContainerMenu {
    private final MatterRefinerBlockEntity be;
    private final Level level;
    private final ContainerData data;

    public MatterRefinerMenu(int id, Inventory inv, FriendlyByteBuf buf) {
        this(id, inv,
                (MatterRefinerBlockEntity) inv.player.level().getBlockEntity(buf.readBlockPos()),
                new SimpleContainerData(8));
    }

    public MatterRefinerMenu(int id, Inventory inv, MatterRefinerBlockEntity be, ContainerData data) {
        super(ModMenuTypes.MATTER_REFINER.get(), id);
        this.be = be;
        ...
        addSlot(new SlotItemHandler(h, MatterRefinerBlockEntity.SLOT_INPUT,    44, 30));
        addSlot(new SlotItemHandler(h, MatterRefinerBlockEntity.SLOT_CATALYST, 44, 52));
        addSlot(new SlotItemHandler(h, MatterRefinerBlockEntity.SLOT_OUTPUT,  122, 40));
        addDataSlots(data);
    }
    ...
    public int feRequired()   { return data.get(7); }
    ...
    public MatterRefinerBlockEntity getBlockEntity() { return be; }
    ...
    public ItemStack quickMoveStack(Player p, int idx) {
        ...
        if (idx >= 36) {
            ...
            int target = -1;
            if (stack.is(ModItems.INACTIVE_DARK_MATTER.get())) target = 36;
            else if (stack.is(Items.DIAMOND) || stack.is(Items.NETHER_STAR)) target = 37;
    ...
    public boolean stillValid(Player p) {
        return stillValid(ContainerLevelAccess.create(level, be.getBlockPos()), p, ModBlocks.MATTER_REFINER.get());
    }
}
