...
/**
 * r181 — <b>Carregador Portátil</b>: bateria turbinada (cap {@value #CAP} FE) que carrega
 * o INVENTÁRIO INTEIRO ao mesmo tempo, rápido, com a própria energia. Recarregue num
 * carregador/reator. Versão premium do {@link BatteryItem}.
 */
public class PortableChargerItem extends Item {
    public static final int CAP = 4_000_000, XFER = 16_000;

    public PortableChargerItem() { super(new Item.Properties().stacksTo(1).rarity(Rarity.RARE)); }

    @Override public @Nullable ICapabilityProvider initCapabilities(ItemStack s, @Nullable CompoundTag n) {
        return TechEnergy.provider(s, CAP, 50_000);
    }
    ...
    @Override public void inventoryTick(ItemStack stack, Level level, Entity entity, int slot, boolean selected) {
        ...
        for (int i = 0; i < inv.getContainerSize() && budget > 0; i++) {
            ...
            if (other == stack || other.isEmpty()
                    || other.getItem() instanceof PortableChargerItem || other.getItem() instanceof BatteryItem) continue;
            int avail = TechEnergy.energy(stack);
            if (avail <= 0) return;
            final int give = Math.min(budget, avail);
    ...
    @Override public void appendHoverText(ItemStack s, @Nullable Level l, List<Component> tip, TooltipFlag f) {
        ...
        tip.add(Component.translatable("tooltip.liberthia.portable_charger").withStyle(net.minecraft.ChatFormatting.GRAY));
    }
